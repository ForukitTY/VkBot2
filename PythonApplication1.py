    ...
    def pars_eldorado(self, itsFirstCall):
        ...
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0'
                          '.4472.135 YaBrowser/21.6.3.757 Yowser/2.5 Safari/537.36'}
        ...
        for iter in range(1, 7):
            ...
            for xx in range(len(EstbBonus)):
                if ItemNameTags[xx].text.find('Apple iPhone') != -1 and ('Yellow') not in ItemNameTags[xx].text:
                    # or ItemNameTags[xx].text.find('Xiaomi')!=-1  or ItemNameTags[xx].text.find('Samsung')!=-1:
